...
class advanalytics(HydraHeadApp):

#wrap all your code in this method and you should be done
    def run(self):
        #-------------------existing untouched code------------------------------------------

        title = '<p style="font-family:sans-serif; color:#207DCE; font-size: 39px; text-align: center;"><b>Advanced analytics</b></p>'
        ...
        df_loaded = dataingestion.readdata()
        # print(df.head())
        df = df_loaded

        vars = list(df.columns.values.tolist())

        st.session_state['pagechoice'] = 'advanced analytics'

        with st.sidebar:
            choose = option_menu("Tiger DataLab", ["Keep in mind", "Show me the steps", "Give me tips", "Give feedback"],
                ...
                "nav-link-selected": {"background-color": "#207DCE"},
            }
            )
        ...
        value = st_click_image_advanalytics()
        if value is None:
            # st.stop()
            pass
            ...
            if value=='EDA':
                ...
                with st.spinner("Analyzing and summarizing dataset and generating dataset profile"):
                    ...
                    with edaprofiling:
                        ...
                        # @st.cache_resource
                        # def gen_profile_report(df2, *report_args, **report_kwargs):
                        #     return df2.profile_report(*report_args, **report_kwargs)

                        # df2 = df.sample(1000)
                        # pr = gen_profile_report(df2, explorative=True, title="Data profile",
                        # dataset={
                        ...
                        # pr = df.sample(1000).profile_report()
                        # st_profile_report(pr)

                        # profile = ProfileReport(df.sample(1000))
                        ...
                        dataprofile.to_file("TigerDataProfile.pdf")
                        st.success("Dataset profile saved to file 'TigerDataProfile.pdf")
                ...
                st.subheader("Value of assets")

                with st.spinner('Please wait while we conduct the asset analysis'):
                    ...
                    st.image('.\gui\images\dashboard_example.png')

                    st.write("")

                    # Spawn a new Quill editor
                    st.subheader("Notes on assets")
                    assetcontent = st_quill(placeholder="Write your notes here", value = st.session_state.assetnotes, key="assetquill")

                    st.session_state.assetnotes = assetcontent

                    st.write("Asset analysis took ", time.time() - start_time, "seconds to run")

            elif value == "Client demographics":

               st.subheader("Explanation here")
               
               with st.spinner('Please wait while we conduct client demographics analysis'):
                    ...
                    # Spawn a new Quill editor
                    st.subheader("Notes on client demographics")
                    clientscontent = st_quill(placeholder="Write your notes here", key="clientsquill")

                    st.session_state.clientsnotes = clientscontent

                    st.write("Client analysis took ", time.time() - start_time, "seconds to run")
